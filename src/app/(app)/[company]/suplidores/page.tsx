...
import { filterSuppliers, hasFilters, parseFilters } from "@/server/supplier-filters";
...
export default async function Page({ params, searchParams }: PageProps<"/[company]/suplidores">) {
  ...
  const shown = filterSuppliers(rows, filters);
  ...
  const columns: Column<SupplierRow>[] = [
    ...
    { key: "type", header: t("col.type"), cell: (r) => tType(r.type) },
    { key: "approval", header: t("col.approval"), cell: (r) => <ApprovalBadge approval={r.approval} /> },
    { key: "sources", header: t("col.sources"), cell: (r) => r.activeSources, className: "tabular-nums" },
    {
      key: "compliance",
